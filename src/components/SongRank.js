...
import { MOCK_TRACK } from "../service/mockData";
...
export default function SongRank() {
  ...
  const randomData = async (TF) => {
    setLoading(true);
    if (TF === 1) {
      setUsers(users.sort((a, b) => (a.duration_ms > b.duration_ms ? 1 : -1)));
    } else if (TF === 2) {
  ...
  const getRanking = async () => {
    ...
    setUsers(MOCK_TRACK.sort((a, b) => (a.popularity > b.popularity ? 1 : -1)));
    setLoading(false);
  };
